...
def determine_season():
    ...
    months_and_day = {"Jan": 31,"Feb": 28, "Mar": 31, "Apr":30, "May": 31, "Jun": 30, "Jul":31, "Aug": 31, "Sep": 30, "Oct":31, "Nov": 30, "Dec": 31}
    

    while True:
        month = input("Enter the month of the year (Jan - Dec):\n")

        if month in months_and_day:
            break
        else:
            print("Sorry, invalid entry")
    ...
    while True:
        ...
        if day.isdigit() and int(day) >0 and int(day)<= months_and_day[month]:
            break
        else:
            print("Sorry, invalid entry")
...
def guess_number():
    ...
    while user_attempt>=1 and not winner:
        if user_attempt==1:
            print("Last chance!\n")
        
        user_input = input(f"Guess a number from 1 to 50. You have {user_attempt} attempts.\n")
        ...
        if int_user_input < 1 or int_user_input > 50:
            print("Sorry, invalid entry")
            continue

        if int_user_input == TARGET:
